...
async function startServer() {
  ...
  app.put("/api/bookings/:id", (req, res) => {
    ...
    if (room) {
      if (status === "confirmed") {
        ...
        // Auto-create approved payment record if booking is confirmed
        const paymentExist = db.payments.find((p: any) => p.booking_id === booking.id);
        if (!paymentExist) {
          db.payments.push({
            id: `pmt-${Date.now()}`,
            ...
            payment_method: "Instan Cash / Transfer (Verifikasi Admin)",
            status: "approved",
    ...
    db.notifications.push({
      ...
      message: status === "confirmed" 
        ? `Selamat! Pemesanan Anda untuk kamar ${room?.name || '#'} telah dikonfirmasi oleh Admin.` 
  ...
  app.post("/api/payments", (req, res) => {
    ...
    const newPayment = {
      ...
      proof_image: pData.proof_image || "https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?auto=format&fit=crop&q=80&w=600",
      status: "pending" as const,
    ...
    db.notifications.push({
      ...
      message: "Bukti pembayaran Anda sebesar Rp " + newPayment.amount.toLocaleString() + " telah kami terima dan sedang diproses verifikasi.",
      is_read: false,
      created_at: new Date().toISOString()
    });
